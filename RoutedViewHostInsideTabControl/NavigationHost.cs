...
namespace RoutedViewHostInsideTabControl
{
    ...
    ///     <para>
    ///         See
    ///         <see href="https://reactiveui.net/docs/handbook/routing/">
    ///             ReactiveUI routing documentation website
    ///         </see>
    ///         for more info.
    ///     </para>
    /// </remarks>
    public class NavigationHost : TransitioningContentControl, IActivatableView, IEnableLogger
    {
        /// <summary>
        ///     <see cref="Router" /> for the <see cref="AvaloniaProperty" /> property.
        /// </summary>
        public static readonly StyledProperty<RoutingState?> RouterProperty =
            AvaloniaProperty.Register<NavigationHost, RoutingState?>(nameof(Router));
        ...
        public static readonly StyledProperty<string?> ViewContractProperty =
            AvaloniaProperty.Register<NavigationHost, string?>(nameof(ViewContract));

        /// <summary>
        ///     <see cref="AvaloniaProperty" /> for the <see cref="DefaultContent" /> property.
        /// </summary>
        public static readonly StyledProperty<object?> DefaultContentProperty =
            ViewModelViewHost.DefaultContentProperty.AddOwner<NavigationHost>();
        ...
        public NavigationHost()
        {
            ...
            {
                IObservable<object?> routerRemoved = this
                                                     .WhenAnyValue(x => x.Router)
                                                     .Where(router => router == null)!
                                                     .Cast<object?>();

                IObservable<string?> viewContract = this.WhenAnyValue(x => x.ViewContract);
        ...
        private void NavigateToViewModel(object? viewModel, string? contract)
        {
            ...
            IViewFor? viewInstance = viewLocator.ResolveView(viewModel, contract);
            if (viewInstance == null)
            {
                this.Log().Warn(contract == null ? $"Couldn't find view for '{viewModel}'. Is it registered? Falling back to default content." : $"Couldn't find view with contract '{contract}' for '{viewModel}'. Is it registered? Falling back to default content.");

                Content = DefaultContent;
                return;
            }

            this.Log().Info(contract == null ? $"Ready to show {viewInstance} with autowired {viewModel}." : $"Ready to show {viewInstance} with autowired {viewModel} and contract '{contract}'.");
